...
#define MOTOR_LEFT_SIN          (0.5f)            /* Cos 60  */
#define MOTOR_RIGHT_SIN         (0.5f)
#define MOTOR_REAR_SIN          (-1.0f)
#define MOTOR_LEFT_COSIN        (-0.866025404f)   /* Sin 300 */
...
void set_motor(float speed, float dir, float trn)
{
    ...
    /* Umrechnug von Grad in Rad */
    dir *= (3.14159265359f / 180.0f);
    
    /* passt diese Formel für unsere Bewegung ? */
    ...
    /* Turnrate wird auf alle Achsen addiert */
    left  += trn;
    right += trn;
    rear  += trn;
...
void set_motor_individual(float left, float right, float rear)
{
    ...
    /* reduziere die Werte so, das maximal 100 % PWM am schellsten Motor sind und die anderen
     * relativ dazu passen */
    ...
    /* Übergabe der werte in das PWM Modul */
